/*		T-MATS -- Turbine_NPSS_TMATS.c
 * % *************************************************************************
 * % written by Jeffryes Chapman based on work by R. May and T. Lavelle
 * % NASA Glenn Research Center, Cleveland, OH
 * % February 15th, 2013
 * %
 * %  This file models the Turbine Element for T-MATS simulation, updated 
 * %  updated to be more similar to the baseline NPSS module of the same name.
 * %  These changes include removing the standard day temperature term from
 * %  the Nc caculation and replacing Ws1in with WIn for Error and iDes s_Wc 
 * %  calculation.
 * % *************************************************************************/
#define S_FUNCTION_NAME  Turbine_NPSS_TMATS
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if (IDes < 0.5 && NDes == 0)
        ...
        NErrorOut = (WIn*sqrt(theta)/delta-WcCalcin)/(WIn*sqrt(theta)/delta) ;
    }
    Test = Wcool[0];
